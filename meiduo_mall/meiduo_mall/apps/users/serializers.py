...
class ResetPasswordSerializer(serializers.ModelSerializer):
    ...
    重置密码序列化器
    """
    # User 模型类中不存在 password2 和 access_token
...
class UserDetailSerializer(serializers.ModelSerializer):
    ...
    # 定义：   手动写与Model匹配         在Meta中写参数fields/exclude去匹配
    # 保存实例：重写create或update方法   已封装了对应方法
...
class AddUserHistorySerializer(serializers.Serializer):
    # Serializer 和 ModelSerializer 的区别
    # 定义：手动写跟 Model 匹配的所有字段；在 Meta 中写参数 fields/exclude 去匹配字段
    # 保存实例：需要重写 (post)Create 或 (put)Update 方法；已经封装不需要重写；调用都是 serializer.save()
    # 校验：都是通过 is_valid 方法，数据保存在 serializer.validated_data 中，或 serializer.error 中
    # 对某个字段自定义验证，重载 validate_<field-name> 方法，参数是 value
    # 对多个字段自定义验证，重载 validate 方法，参数是 attrs 字典
    # 对所有字段自定义验证，参数是 value，在字段中添加 validators = [my_validator]
    ...
    def create(self, validated_data):
        ...
        # 清除sku_id在redis中的记录
        # lrem(name, count, value)  l remove
